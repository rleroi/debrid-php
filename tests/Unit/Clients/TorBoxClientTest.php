...
class TorBoxClientTest extends TestCase
{
    ...
    private TorBoxClient $client;
    private ClientInterface $mockHttp;

    protected function setUp(): void
    {
        $this->mockHttp = Mockery::mock(ClientInterface::class);
        $this->client = new TorBoxClient(self::TEST_TOKEN, $this->mockHttp);
    }
    ...
    {
        $client = new TorBoxClient(self::TEST_TOKEN);
        $this->assertInstanceOf(TorBoxClient::class, $client);
    }

    public function testConstructorWithoutToken(): void
    {
        $client = new TorBoxClient(null);
        $this->assertInstanceOf(TorBoxClient::class, $client);
    }

    public function testSetToken(): void
    {
        $client = new TorBoxClient(null, $this->mockHttp);
        ...
        $this->mockHttp->shouldReceive('request')
            ->once()
            ->with('GET', 'torrents/checkcached', Mockery::any())
            ->andReturn(new Response(200, [], json_encode(['success' => true, 'data' => []])));
    ...
    public function testRequestWithApiErrorThrowsException(): void
    {
        $this->mockHttp->shouldReceive('request')
            ->once()
            ->andReturn(new Response(200, [], json_encode(['success' => false, 'detail' => 'API error'])));
    ...
    {
        // This test is no longer applicable since we don't add magnets automatically
        // TorBoxClient only checks cache, doesn't add magnets
        $this->markTestSkipped('TorBoxClient does not add magnets automatically');
    }
    ...
    public function testAddMagnet(): void
    {
        ...
        $this->mockHttp->shouldReceive('request')
            ->once()
            ->with('POST', 'torrents/createtorrent', Mockery::any())
            ->andReturn($response);
